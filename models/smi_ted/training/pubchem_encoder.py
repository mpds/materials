...
class Encoder:
    ...
    def __init__(self, max_length=500, add_bos=True, add_eos=True, feature_size=32):
        self.vocab_encoder = torch.load(
            "pubchem_canon_zinc_final_vocab_sorted_curated.pth"
        )
    ...
    def decode(self, token_ids):
        ...
        tokens = [self.id2char.get(id, "<unk>") for id in token_ids]
        # Filter out special tokens
        filtered = [t for t in tokens if t not in ["<bos>", "<eos>", "<pad>", "<mask>"]]
    ...
    def encode(self, char):
        ...
        return torch.tensor(
            [self.char2id.get(word, self.char2id["<unk>"]) for word in char]
        )
    ...
    def process_text(self, text):
        ...
        if len(self.bucket2) < self.b2_max and len(self.b2_cache) > 0:
            ...
            outbucket2 = [self.bucket2.pop() for item in range(len(self.bucket2))] + [
                self.b2_cache.pop() for i in range(range2)
            ]
        else:
            outbucket2 = [self.bucket2.pop() for item in range(len(self.bucket2))]
        ...
        if len(self.bucket3) < self.b3_max and len(self.b3_cache) > 0:
            ...
            outbucket3 = [self.bucket3.pop() for item in range(len(self.bucket3))] + [
                self.b3_cache.pop() for i in range(range3)
            ]
        else:
            outbucket3 = [self.bucket3.pop() for item in range(len(self.bucket3))]
    ...
    def pack_tensors(self, tokens):
        ...
        array = torch.nn.utils.rnn.pad_sequence(
            array_ids, batch_first=True, padding_value=self.pad
        )
    ...
    def process_with_output(self, text, output_text):
        """Process text with corresponding output text for paired mode"""
        # print("\n=== Inside process_with_output ===")  # DEBUG
        # print(f"First input text: {text[0]['text']}")
        # print(f"First output text: {output_text[0]['text']}")

        input_buckets = self.process_text(text)
        output_buckets = self.process_text(output_text)

        # DEBUG: Print bucket contents
        # print(f"\nGenerated {len(input_buckets)} input buckets")
        # print(f"Generated {len(output_buckets)} output buckets")

        # Ensure input and output have the same number of buckets
        min_buckets = min(len(input_buckets), len(output_buckets))
        input_buckets = input_buckets[:min_buckets]
        output_buckets = output_buckets[:min_buckets]
        ...
        for i in range(min_buckets):
            input_tokens = input_buckets[i]
            output_tokens = output_buckets[i]

            # Make sure we have tokens in both buckets
            if len(input_tokens) > 0 and len(output_tokens) > 0:
                # Truncate to same length to ensure alignment
                min_len = min(len(input_tokens), len(output_tokens))
                input_tokens = input_tokens[:min_len]
                output_tokens = output_tokens[:min_len]

                array, target, array_ids, lgt = self.pack_tensors(input_tokens)
                output_array_ids = self.encoder(output_tokens)

                arrays.append(array)
                targets.append(target)
                arrays_ids.append(array_ids)
                output_arrays_ids.append(output_array_ids)
                lengths.append(lgt)

        return arrays, targets, arrays_ids, lengths, output_arrays_ids
